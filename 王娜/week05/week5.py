"""
字符级单向 Transformer 自回归语言模型训练脚本（含 PPL 计算）。
使用下三角 causal attention mask，确保每个位置只能 attend 到之前的位置。
用法:
...
class LM(nn.Module):
    def __init__(self, vocab_size, embed_dim, hidden_dim, num_layers, nhead, dropout, max_len=512):
        super().__init__()
        ...
        # 预计算 causal mask，避免每次 forward 重复创建
    ...
    def forward(self, x):
        ...
        h = self.dropout(self.embed(x) + self.pos_embed(pos))

        # 下三角 causal mask: True 表示"不允许attend"
        # 对角线(diagonal=1)及以上为 True，确保位置 j 只能看到 0..j
        mask = self.mask[:T, :T]
...
def main():
    ...
    parser.add_argument("--save",       default="my_model.pt")
